...
func chat(messages: Array, params: Dictionary = {}) -> GameAIResult:
...
func chat_system(system: String, user_message: String, params: Dictionary = {}) -> GameAIResult:
	# Convenience: single chat with system prompt
	return chat([{"role": "system", "content": system}, {"role": "user", "content": user_message}], params)


func _send_chat(provider: String, messages: Array, params: Dictionary) -> GameAIResult:
	match provider:
		"anthropic":
		...
		_:
			return GameAIResult.new(false, null, {"code": -1, "message": "Unknown provider: " + provider})


# === Anthropic Claude ===

func _anthropic_chat(messages: Array, params: Dictionary) -> GameAIResult:
...
func _parse_anthropic_response(response_text: String) -> GameAIResult:
...
func _openai_chat(messages: Array, params: Dictionary) -> GameAIResult:
	var api_key = _config.get_provider_key("openai")
	if api_key == "":
		return GameAIResult.new(false, null, {"code": -2, "message": "OpenAI API key not configured"})
...
func _parse_openai_response(response_text: String) -> GameAIResult:
	var json = JSON.parse_string(response_text)
	if json == null:
		return GameAIResult.new(false, null, {"code": -3, "message": "Failed to parse OpenAI response"})
	if json.has("error"):
		return GameAIResult.new(false, null, {"code": -4, "message": json.error.message})
...
func _minimax_chat(messages: Array, params: Dictionary) -> GameAIResult:
	var api_key = _config.get_provider_key("minimax")
	if api_key == "":
		return GameAIResult.new(false, null, {"code": -2, "message": "Minimax API key not configured"})
...
func _parse_minimax_response(response_text: String) -> GameAIResult:
	var json = JSON.parse_string(response_text)
	if json == null:
		return GameAIResult.new(false, null, {"code": -3, "message": "Failed to parse Minimax response"})
	if json.has("base_resp") and json.base_resp.has("status_code"):
		if json.base_resp.status_code != 0:
			return GameAIResult.new(false, null, {"code": -4, "message": json.base_resp.status_msg})
...
func generate_code(task: String, language: String = "gdscript") -> GameAIResult:
...
func explain_code(code: String) -> GameAIResult:
...
func npc_say(npc_id: String, player_message: String) -> GameAIResult:
	# Get NPC response to player message
	if not _npc_contexts.has(npc_id):
		return GameAIResult.new(false, null, {"code": -1, "message": "NPC not initialized: " + npc_id})
...
func generate_quest(theme: String, difficulty: String = "medium") -> GameAIResult:
...
func generate_item_name(item_type: String, rarity: String = "common") -> GameAIResult:
...
func generate_dungeon_layout(size: String = "medium", theme: String = "dungeon") -> GameAIResult:
...
func ask_about_game(game_context: String, question: String) -> GameAIResult:
...
func chat_stream(messages: Array, params: Dictionary = {}) -> GameAIResult:
	...
	match provider:
		...
		_:
			return GameAIResult.new(false, null, {"code": -1, "message": "Streaming not supported for " + provider})


func _anthropic_stream(messages: Array, params: Dictionary) -> GameAIResult:
	var api_key = _config.get_provider_key("anthropic")
	if api_key == "":
		return GameAIResult.new(false, null, {"code": -2, "message": "Anthropic API key not configured"})
